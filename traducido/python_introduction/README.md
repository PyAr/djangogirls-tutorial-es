...
Si deseas mostrar sólo el primer número, puedes hacerlo mediante el uso de **indexes** (en español, índices). Un índice es el número que te dice dónde en una lista aparece un ítem. Los programadores prefieren comenzar a contar desde 0, por lo tanto el primer objeto en tu lista esta en el indice 0, el próximo esta en el 1, y así sucesivamente. Intenta esto:
...
Para borrar algo de tu lista necesitas usar **indices** como aprendimos anteriormente y la declaración **del** (del es una abreviación de delete). Vamos a tratar de ejemplificar esto y reforzaar lo que aprendimos anteriormente; vamos a borrar el primer número de nuestra lista.
...
> Para lectores en casa: esta parte está cubierta en el video [Python Basics: Saving files and "If" statement](https://www.youtube.com/watch?v=dOAg6QVAxyk) (en Inglés).
...
Obviamente, eres una programadora de Python bastante experimentada ahora, asi que siéntente libre de escribir algo del código que has aprendido hoy
...
Con el archivo guardado, ¡es hora de ejecutarlo! Utilizando las habilidades que has aprendido en la sección de línea de comandos, utiliza la terminal para **cambiar los directorios** e ir al escritorio. (**Nota**:  reemplaza `<your_name>` incluyendo los signos `<` y `>` con tu nombre de usuario)
...
Ahora, ¿qué crees que suceda si escribes otro nombre ahí? (Ni Ola ni Sonja) Inténtalo y ve si tienes razón. Debería imprimir esto:
...
> Para lectores en casa: esta parte está cubierta en el video [Python Basics: For Loop](https://www.youtube.com/watch?v=aEA6Rc86HF0) (en Inglés).
